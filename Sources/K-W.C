...
void main(int argc,char *argv[])
{
  FILE *stream;
  int i,j,l;
  double num,t[N]={0,},k,temp=0,alpha,chi;

  clrscr();

  if (argc<=1) {
    puts("Usage : k-w datafile");
    exit(EXIT_FAILURE);
  }
  ...
  for (i=0;i<col;i++) temp+=pow(t[i],2)/(double)row;

  k=12./(n*(n+1))*temp-3*(n+1);

  printf("\t\t ** Kruskal-Wallis Tets **\n\n");
  printf("\t\t   H0 : M1");
  ...
  for (i=0;i<row;i++)
	...
	if ((i*(col)+j+1)==x[l].id) {
	  if (((x[l].id-1)%col)==0)
	    printf("\t%6.1f  %4.1f",x[l].x,x[l].no);
	  else if (((x[l].id-1)%col)==(col-1))
	    printf(" %6.1f  %4.1f\n",x[l].x,x[l].no);
	  else printf(" %6.1f  %4.1f",x[l].x,x[l].no);
	  break;
	}
  printf("      -------");
  for (i=1;i<col;i++) printf("----------------");
