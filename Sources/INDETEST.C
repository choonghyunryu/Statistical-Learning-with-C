/* Program : INDETEST.C ver 0.1                       */
/* Author  : Ryu choong hyun                          */
/* Date    : 94.8.15.		                      */
/* Note    : R*C Contingency Tabel's Independent Test */
...
int idd_test(double alpha,double *cv,double *chi);
double nor_point(double point);
double chi_point(int df, double point);
void rc_table(void);

int row,col;
double o[FACTOR][FACTOR],e[FACTOR][FACTOR],
       e_i[FACTOR],e_j[FACTOR],total;
...
void main(int argc,char *argv[])
{
  ...
  printf("** Independent Test (R*C Contingency Tabel) **\n\n");
  printf("  Significance Level = ");
  scanf("%lf",&alpha);

  h0=idd_test(alpha,&cv,&chi);
  rc_table();
  ...
  else printf("tNull Hypothesis(H0) is Reject\n");
  getch();
...
int idd_test(double alpha,double *cv,double *chi)
{
  ...
  for (i=0;i<row;i++) {
    sum=0;
    for (j=0;j<col;j++) sum+=o[i][j];
    e_i[i]=sum;
  }

  for (i=0;i<col;i++) {
    sum=0;
    for (j=0;j<row;j++) sum+=o[j][i];
    e_j[i]=sum;
  }

  for (i=0;i<row;i++)
    for (j=0;j<col;j++) e[i][j]=(e_i[i]*e_j[j])/total;

  *cv=0;
  for (i=0;i<row;i++)
    for (j=0;j<col;j++) *cv+=pow(o[i][j]-e[i][j],2)/e[i][j];

  *chi=chi_point((row-1)*(col-1),alpha);
...
void rc_table(void)
{
  ...
  printf("\n << R*C Contingency Tabel >>\n     ");
  for (i=0;i<col;i++) printf("      C%d",i+1);
  printf("    Total\n");
  ...
  for (i=0;i<row;i++) {
    ...
    for (j=0;j<col;j++) printf("   %5.f",o[i][j]);
    printf("  %5.f\n   ",e_i[i]);
    for (j=0;j<col;j++) printf("    %5.1f",e[i][j]);
  }

  printf("\nTotal");
  for (i=0;i<col;i++) printf("   %5.f",e_j[i]);
  printf("  %5.f\n",total);
}
