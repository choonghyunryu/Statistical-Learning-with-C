...
int md_one_test2(char opt,char job,int n1,int n2,double s1,double s2,
		 double mean1,double mean2,double alpha,double *cv);
int md_two_test2(int n1,int n2,double s1,double s2,double mean1,double mean2,
		 double dif,double alpha,double *cv1,double *cv2,char opt);
...
void main(void)
{
  char opt;
  int n1,n2,h0;
  double mean1,mean2,s1,s2,dif,alpha,cv1,cv2;

  clrscr();
  printf("\t** Statistical Hypothesis Testing Difference Between\xe6 **\n\n");
  printf("\t\t1. H0 : \xe6%d \xf3 \xe6%d"
	 "  H1 : \xe6%d > \xe6%d (Same \xe5\xfd)\n",1,2,1,2);
  printf("\t\t2. H0 : \xe6%d \xf3 \xe6%d"
	 "  H1 : \xe6%d > \xe6%d (Unsame \xe5\xfd)\n",1,2,1,2);
  printf("\t\t3. H0 : \xe6%d \xf2 \xe6%d"
	 "  H1 : \xe6%d < \xe6%d (Same \xe5\xfd)\n",1,2,1,2);
  printf("\t\t4. H0 : \xe6%d \xf2 \xe6%d"
	 "  H1 : \xe6%d < \xe6%d (Unsame \xe5\xfd)\n",1,2,1,2);
  printf("\t\t5. H0 : \xe6%d-\xe6%d=D"
	 "  H1 : \xe6%d-\xe6%d\xd8%c (Same \xe5\xfd)\n",1,2,1,2,'D');
  printf("\t\t6. H0 : \xe6%d-\xe6%d=D"
	 "  H1 : \xe6%d-\xe6%d\xd8%c (Unsame \xe5\xfd)\n\n",1,2,1,2,'D');
  do
  switch (opt=getch()) {
    case '1':
    case '2':
  ...
  if (opt=='5' || opt=='6') {
    if (opt=='5')
      h0=md_two_test2(n1,n2,s1,s2,mean1,mean2,dif,alpha,&cv1,&cv2,'0');
    else
      h0=md_two_test2(n1,n2,s1,s2,mean1,mean2,dif,alpha,&cv1,&cv2,'1');
    printf("\n\t\tLower Cirtical Value = %lf\n",cv1);
    printf("\t\tUpper Cirtical Value = %lf\n",cv2);
  }
  else {
    if (opt=='1' || opt=='3')
      h0=md_one_test2(opt,'0',n1,n2,s1,s2,mean1,mean2,alpha,&cv1);
    else
      h0=md_one_test2(opt,'1',n1,n2,s1,s2,mean1,mean2,alpha,&cv1);
    printf("\n\t\tCirtical Value = %lf\n",cv1);
  }
		 ...
		 double mean1,double mean2,double alpha,double *cv)
{
  ...
  m=pow(s1/n1+s2/n2,2)/(pow(s1/n1,2)/(n1-1)+pow(s2/n2,2)/(n2-1));
  if (frac(pow(s1/n1+s2/n2,2)/(pow(s1/n1,2)/(n1-1)+pow(s2/n2,2)/(n2-1)))>=.5)
    m++;
  ...
  switch (opt) {
    case '1':
    case '2': if (n1>=30 && n2>=30) *cv=nor_point(1-alpha)*sqrt(s1/n1+s2/n2);
	      else if (job=='0')
		*cv=t_point(n1+n2-2,1-alpha)*sqrt(sp*(1./n1+1./n2));
	      else *cv=t_point(m,1-alpha)*sqrt(s1/n1+s2/n2);
	      break;
    case '3':
    case '4': if (n1>=30 && n2>=30) *cv=nor_point(alpha)*sqrt(s1/n1+s2/n2);
	      else if (job=='0')
		*cv=t_point(n1+n2-2,alpha)*sqrt(sp*(1./n1+1./n2));
	      else *cv=t_point(m,alpha)*sqrt(s1/n1+s2/n2);
	      break;
    default : break;
  }
		 ...
		 double dif,double alpha,double *cv1,double *cv2,char opt)
{
  ...
  if (n1>=30 && n2>=30) {
    *cv1=dif+nor_point(alpha/2)*sqrt(s1/n1+s2/n2);
    *cv2=dif+nor_point(1-alpha/2)*sqrt(s1/n1+s2/n2);
  }
  else {
    if (opt=='0') {
      *cv1=dif-t_point(n1+n2-2,alpha/2)*sqrt(sp*(1./n1+1./n2));
      *cv2=dif+t_point(n1+n2-2,alpha/2)*sqrt(sp*(1./n1+1./n2));
    }
    else {
      *cv1=dif-t_point(m,alpha/2)*sqrt(s1/n1+s2/n2);
      *cv2=dif+t_point(m,alpha/2)*sqrt(s1/n1+s2/n2);
    }
  }
