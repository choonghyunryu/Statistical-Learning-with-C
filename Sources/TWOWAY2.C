...
void estmean1(int df,double mean,double mse,double alpha,double *l,double *u);
void estmean2(int df,double mean,double mse,double alpha,double *l,double *u);
void infolevel1(int level1,double *total,double *mean);
void infolevel2(int level2,double *total,double *mean);
double mu_ij(int row,int col);
double get_sst(double mean);
double get_ssa(double mean);
double get_ssb(double mean);
double get_ssab(void);

int row,col,n;
double data[FACTOR1][FACTOR2][FACTOR3],total,l1[FACTOR1],u1[FACTOR1],
       l2[FACTOR2],u2[FACTOR2],t_level1[FACTOR1],m_level1[FACTOR1],
       t_level2[FACTOR2],m_level2[FACTOR2];

void main(int argc,char *argv[])
{
  FILE *stream;
  int i,j,k,dft,dfa,dfb,dfa_b,dfe;
  double num,mean,sst,ssa,ssb,ssab,ssa_b,sse,msa,msb,msa_b,mse,
         fa,fb,fa_b,alpha,fsa,fsb,fsa_b,mu;
  ...
  mean=total/(row*col*n);

  for (i=0;i<row;i++) infolevel1(i,&t_level1[i],&m_level1[i]);
  for (i=0;i<col;i++) infolevel2(i,&t_level2[i],&m_level2[i]);

  printf("\t*** Analysis of Variance (Two-Way) ***\n\n");
  printf("<< Data Structure >>\n");
  printf("** Factor1 **\n");
  printf("Source          ");
  for (i=0;i<row;i++) printf("Level%d     ",i+1);
  printf(" Sum\n");
  printf("Total        ");
  for (i=0;i<row;i++) printf("%10.5f ",t_level1[i]);
  printf("%10.5f\n",total);
  printf("Mean         ");
  for (i=0;i<row;i++) printf("%10.5f ",m_level1[i]);
  printf("%10.5f\n",mean);
  ...
  printf(" Sum\n");
  printf("Total        ");
  for (i=0;i<col;i++) printf("%10.5f ",t_level2[i]);
  printf("%10.5f\n",total);
  printf("Mean         ");
  for (i=0;i<col;i++) printf("%10.5f ",m_level2[i]);
  printf("%10.5f\n",mean);
  getch();

  printf("\n\n<< Analysis of Variance Model >>\n");
  printf("Xijk = %lf + Ai + Bj + Iij + Eij\n",mean);

  for (i=0;i<row;i++) printf("A%d = %lf\n",i,m_level1[i]-mean);
  for (i=0;i<col;i++) printf("B%d = %lf\n",i,m_level2[i]-mean);

  for (i=0;i<row;i++)
    for (j=0;j<col;j++) {
      mu=mu_ij(i,j);
      printf("I%d%d = %lf\n",i,j,mu-m_level1[i]-m_level2[j]+mean);
    }
  ...
  printf("Factor1        %d     %10.3f\t%10.3f  %10.3f\n",dfa,ssa,msa,fa);
  printf("Factor2        %d     %10.3f\t%10.3f  %10.3f\n",dfb,ssb,msb,fb);
  printf("Interactoin    %d     %10.3f\t%10.3f  %10.3f\n",
         dfa_b,ssa_b,msa_b,fa_b);
  ...
  printf("\n** Factor1 **\n");
  printf("H0 : \xe6%d",1);
  for (i=1;i<row;i++) printf(" = \xe6%d",i+1);
  printf("\n** Factor2 **\n");
  printf("H0 : \xe6%d",1);
  ...
  fsa=f_point(dfa,dfe,alpha);
  fsb=f_point(dfb,dfe,alpha);
  fsa_b=f_point(dfa_b,dfe,alpha);

  printf("\nFactor1 : F-statics = %lf\n",fsa);
  if(fa<=fsa) printf("Factor 1 H0 is Accept\n\n");
  else printf("Factor1 H0 is Reject\n\n");

  printf("Factor2 : F-statics = %lf\n",fsb);
  if(fb<=fsb) printf("Factor2 H0 is Accept\n\n");
  else printf("Factor2 H0 is Reject\n\n");

  printf("Interaction Effect : F-statics = %lf\n",fsa_b);
  if(fa_b<=fsa_b) printf("Factor2 H0 is Accept\n\n");
  else printf("Factor2 H0 is Reject\n\n");
  ...
  printf("\n** Factor1 **\n");
  for (i=0;i<row;i++) {
    estmean1(dfe,m_level1[i],mse,alpha,&l1[i],&u1[i]);
    printf("%lf \xf3 \xe6%d \xf3 %lf\n",l1[i],i+1,u1[i]);
  }
  printf("\n** Factor2 **\n");
  for (i=0;i<col;i++) {
    estmean2(dfe,m_level2[i],mse,alpha,&l2[i],&u2[i]);
    printf("%lf \xf3 \xe6%d \xf3 %lf\n",l2[i],i+1,u2[i]);
  }
  getch();
}
...
void infolevel1(int level1,double *total,double *mean)
{
  ...
  for (i=0;i<col;i++)
    for (j=0;j<n;j++) sum+=data[level1][j][i];
...
void infolevel2(int level2,double *total,double *mean)
{
  ...
  for (i=0;i<row;i++)
    for (j=0;j<n;j++) sum+=data[i][j][level2];
...
double get_sst(double mean)
{
  ...
  for (i=0;i<row;i++)
    for (j=0;j<n;j++)
      for (k=0;k<col;k++) sum+=pow(data[i][j][k]-mean,2);
...
double get_ssa(double mean)
{
  ...
  for (i=0;i<row;i++) sum+=row*col*pow(m_level1[i]-mean,2);
...
double get_ssb(double mean)
{
  ...
  for (i=0;i<col;i++) sum+=row*n*pow(m_level2[i]-mean,2);
...
double get_ssab(void)
{
  ...
  for (i=0;i<row;i++)
    for (j=0;j<col;j++) {
      sum=0;
      for (k=0;k<n;k++) sum+=data[i][k][j];
      mult+=pow(sum,2);
    }

  return mult/n-pow(total,2)/(row*col*n);
}
...
void estmean1(int df,double mean,double mse,double alpha,double *l,double *u)
{
  ...
  *l=mean-t*sqrt(mse/(row*col));
  *u=mean+t*sqrt(mse/(row*col));
}
...
void estmean2(int df,double mean,double mse,double alpha,double *l,double *u)
{
  ...
  *l=mean-t*sqrt(mse/(row*n));
  *u=mean+t*sqrt(mse/(row*n));
}
