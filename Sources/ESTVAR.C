/* Program : ESTVAR.C ver 0.1              */
/* Author  : Ryu choong hyun               */
/* Date    : 94.7.18.		           */
/* Note    : Interval Estimate of Variance */
...
void est_var(int n,double s,double alpha,double *l,double *u);

void main(void)
{
  int n;
  double s,alpha,lower,upper;

  clrscr();
  printf("  *** Inverval Estimation of Variance ***\n\n");
  printf("\t     Number of Sample   = ");
  scanf("%d",&n);
  printf("\t     Variance of Sample = ");
  scanf("%lf",&s);
  printf("\t     Condidence Level = ");
  scanf("%lf",&alpha);

  est_var(n,s,alpha,&lower,&upper);
  printf("\n\n    N      S\xfd      C.L.\t( %4.2f% C.I )\n",alpha);
  printf("%5d%10.3f%8.2f %10.3f%10.3f\n",n,s,alpha*100,lower,upper);
...
void est_var(int n,double s,double alpha,double *l,double *u)
{
  *l=(n-1)*s/chi_point(n-1,(1-alpha)/2);
  *u=(n-1)*s/chi_point(n-1,1-(1-alpha)/2);
}
