...
int get_run(int n);
int run_test(double alpha,double run,char opt);
...
void main(int argc,char *argv[])
{
  FILE *stream;
  char opt;
  int i,j,n=0,num,run,h0;
  double alpha;

  clrscr();

  if (argc<=1) {
    puts("Usage : runtest datafile");
    exit(EXIT_FAILURE);
  }
  ...
  printf("\t\t   ** Run Test **\n\n");
  printf("\t\t1. H0 : A B is Random  H1 : Many Run\n");
  printf("\t\t2. H0 : A B is Random  H1 : Few  Run\n");
  printf("\t\t3. H0 : A B is Random  H1 : A B is not Random\n\n");
  do
  switch (opt=getch()) {
    case '1':
    case '2':
...
int run_test(double alpha,double run,char opt)
{
  ...
  dev=sqrt((2.*n1*n2*(2.*n1*n2-n1-n2))/(pow(n1+n2,2)*(n1+n2+1)));
  z=(run-mean)/dev;
