/* Program : ESTMEAN.C ver 0.1         */
/* Author  : Ryu choong hyun           */
/* Date    : 94.7.18.		       */
/* Note    : Interval Estimate of Mean */
...
void est_mean1(int n,double mean,double var,double alpha,double *l,double *u);
void est_mean2(int n,double mean,double s,double alpha,double *l,double *u);
int mean_sample(double alpha,double var,double d);

void main(void)
{
  char opt;
  int n;
  double mean,var,alpha,lower,upper,d;

  clrscr();
  printf("\t     *** Inverval Estimation of \xe6 ***\n\n");
  printf("\t1.Known \xe5\xfd  2.Unknown \xe5\xfd  3.Numbur of Sample\n\n");

  do
  switch (opt=getch()) {
    case '1': printf("\t     Number of Sample = ");
	      scanf("%d",&n);
	      ...
	      scanf("%lf",&alpha);
	      est_mean1(n,mean,var,alpha,&lower,&upper);
	      break;
    case '2': printf("\t     Number of Sample  = ");
	      scanf("%d",&n);
	      printf("\t     Mean of Sample    = ");
	      scanf("%lf",&mean);
	      printf("\t     Sample's Variance = ");
	      scanf("%lf",&var);
	      printf("\t     Condidence Level  = ");
	      scanf("%lf",&alpha);
	      est_mean2(n,mean,var,alpha,&lower,&upper);
    case '3': printf("\t\t\xe5\xfd\t\t = ");
	      scanf("%lf",&var);
	      printf("\t\tDistance from \xe6  = ");
	      scanf("%lf",&d);
	      printf("\t\tCondidence Level = ");
	      scanf("%lf",&alpha);
	      n=mean_sample(alpha,var,d);
	      printf("\n\n\t\tNumber of Sample = %d\n",n);
    default : break;
  } while (!(opt=='1' || opt=='2' || opt=='3'));

  if (opt=='2')
    printf("    N     MEAN       S\xfd      C.L.\t( %4.2f% C.I )\n",alpha);
  else if (opt=='1')
    printf("    N     MEAN       \xe5\xfd      C.L.\t( %4.2f% C.I )\n",alpha);
  if (opt!='3')  printf("%5d%10.3f%10.3f%8.2f %10.3f%10.3f\n",
			 n,mean,var,alpha*100,lower,upper);
  getch();
}
...
void est_mean1(int n,double mean,double var,double alpha,double *l,double *u)
{
  ...
  *l=mean-fabs(z)*sqrt(var/n);
  *u=mean+fabs(z)*sqrt(var/n);
}
...
void est_mean2(int n,double mean,double s,double alpha,double *l,double *u)
{
  ...
  t=(n>=30) ? nor_point((1-alpha)/2) : (alpha<=.5) ? -t_point(n-1,alpha/2) :
				       t_point(n-1,(1-alpha)/2);
  *l=mean-fabs(t)*sqrt(s/n);
  *u=mean+fabs(t)*sqrt(s/n);
}
...
int mean_sample(double alpha,double var,double d)
{
  ...
  return ceil(pow(z,2)*var/pow(d,2));
}
