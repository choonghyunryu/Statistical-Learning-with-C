...
void estmean(int df,double mean,double mse,double alpha,double *l,double *u);
void infolevel1(int level1,double *total,double *mean,double *ss);
void infolevel2(int level2,double *total,double *mean,double *ss);
double get_sst(double mean);
double get_ssa(double mean);
double get_ssb(double mean);

int row,col;
double data[FACTOR][FACTOR],total,l1[FACTOR],u1[FACTOR],l2[FACTOR],u2[FACTOR],
       t_level1[FACTOR],m_level1[FACTOR],ss_level1[FACTOR],
       t_level2[FACTOR],m_level2[FACTOR],ss_level2[FACTOR];

void main(int argc,char *argv[])
{
  FILE *stream;
  int i,j,dft,dfa,dfb,dfw;
  double num,mean,sst,ssa,ssb,ssw,msa,msb,msw,fa,fb,alpha,fsa,fsb;
  ...
  mean=total/(row*col);
  for (i=0;i<row;i++) infolevel1(i,&t_level1[i],&m_level1[i],&ss_level1[i]);
  for (i=0;i<col;i++) infolevel2(i,&t_level2[i],&m_level2[i],&ss_level2[i]);
  ...
  printf("%10.5f\n",mean);
  printf("Sum Square   ");
  for (i=0;i<row;i++) printf("%10.5f ",ss_level1[i]);
  ...
  printf("%10.5f\n",mean);
  printf("Sum Square   ");
  for (i=0;i<col;i++) printf("%10.5f ",ss_level2[i]);

  printf("\n\n<< Analysis of Variance Model >>\n");
  printf("Xij = %lf + Ai + Bj +Eij\n",mean);
  ...
  fsa=f_point(dfa,dfw,alpha);
  fsb=f_point(dfb,dfw,alpha);
  ...
  printf("\n** Factor1 **\n");
  for (i=0;i<row;i++) {
    estmean(dfw,m_level1[i],msw,alpha,&l1[i],&u1[i]);
    printf("%lf \xf3 \xe6%d \xf3 %lf\n",l1[i],i+1,u1[i]);
  }
  printf("\n** Factor2 **\n");
  for (i=0;i<col;i++) {
    estmean(dfw,m_level2[i],msw,alpha,&l2[i],&u2[i]);
    printf("%lf \xf3 \xe6%d \xf3 %lf\n",l2[i],i+1,u2[i]);
  }
  getch();
}
...
void infolevel1(int level1,double *total,double *mean,double *ss)
{
  ...
  for (i=0;i<col;i++) sum+=data[level1][i];

  *total=sum;
  *mean=sum/col;

  for (i=0;i<col;i++) sos+=pow(data[level1][i]-*mean,2);
...
void infolevel2(int level2,double *total,double *mean,double *ss)
{
  ...
  for (i=0;i<row;i++) sum+=data[i][level2];

  *total=sum;
  *mean=sum/row;

  for (i=0;i<row;i++) sos+=pow(data[i][level2]-*mean,2);
...
double get_ssa(double mean)
{
  ...
  for (i=0;i<row;i++) sum+=col*pow(m_level1[i]-mean,2);
...
double get_ssb(double mean)
{
  ...
  for (i=0;i<col;i++) sum+=row*pow(m_level2[i]-mean,2);
...
void estmean(int df,double mean,double mse,double alpha,double *l,double *u)
{
  ...
  *l=mean-t*sqrt(mse/df);
  *u=mean+t*sqrt(mse/df);
}
