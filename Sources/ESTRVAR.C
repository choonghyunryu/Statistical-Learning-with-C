/* Program : ESTRVAR.C ver 0.1                      */
/* Author  : Ryu choong hyun                        */
/* Date    : 94.7.18.		                    */
/* Note    : Interval Estimate of Ratio of Variance */
...
void est_rvar(int n1,int n2,double s1,double s2,
	      double alpha,double *l,double *u);

void main(void)
{
  int n1,n2;
  double s1,s2,alpha,lower,upper;

  clrscr();
  printf("  *** Inverval Estimation of Ratio of Variance ***\n\n");
  printf("\t     Number of Sample(N1)   = ");
  scanf("%d",&n1);
  printf("\t     Variance of Sample(S1\xfd) = ");
  scanf("%lf",&s1);
  printf("\t     Number of Sample(N2)   = ");
  scanf("%d",&n2);
  printf("\t     Variance of Sample(S2\xfd) = ");
  scanf("%lf",&s2);
  printf("\t     Condidence Level = ");
  scanf("%lf",&alpha);

  est_rvar(n1,n2,s1,s2,alpha,&lower,&upper);
  printf("\n\n    N      S\xfd      C.L.\t( %4.2f% C.I )\n",alpha);
  printf("%5d%10.3f%8.2f %10.3f%10.3f\n",n1,s1,alpha*100,lower,upper);
...
void est_rvar(int n1,int n2,double s1,double s2,
	      double alpha,double *l,double *u)
{
  *l=s2/s1*f_point(n1-1,n2-1,1-(1-alpha)/2);
  *u=s2/s1*f_point(n1-1,n2-1,(1-alpha)/2);
}
