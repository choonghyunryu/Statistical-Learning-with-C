...
int m_one_test1(char opt,int n,double mu,double var,double mean,
		double alpha,double *cv);
int m_two_test1(int n,double mu,double var,double mean,
...
void main(void)
{
  ...
  if (opt=='3') {
    h0=m_two_test1(n,mu,var,mean,alpha,&cv1,&cv2);
    printf("\n\t\tLower Cirtical Value = %lf\n",cv1);
    printf("\t\tUpper Cirtical Value = %lf\n",cv2);
  }
  else {
    h0=m_one_test1(opt,n,mu,var,mean,alpha,&cv1);
    printf("\n\t\tCirtical Value = %lf\n",cv1);
  }
...
{
  switch (opt) {
    case '1': *cv=mu+nor_point(1-alpha)*sqrt(var/n);
	      break;
    case '2': *cv=mu+nor_point(alpha)*sqrt(var/n);
	      break;
    default : break;
  }
...
int m_two_test1(int n,double mu,double var,double mean,
		double alpha,double *cv1,double *cv2)
{
  *cv1=mu+nor_point(alpha/2)*sqrt(var/n);
  *cv2=mu+nor_point(1-alpha/2)*sqrt(var/n);
